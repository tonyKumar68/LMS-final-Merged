...
function CreateLecture() {
    ...
    const createLectureHandler = async () => {
      if (isLive && !liveStartTime) {
        toast.error("Please select live start time if enabling live session");
        return;
      }
      setLoading(true)
      try {
        const result = await axios.post(serverUrl + `/api/course/createlecture/${courseId}` ,{lectureTitle, isLive, liveStartTime: liveStartTime || null} , {withCredentials:true})
        ...
        <div className="mb-4">
          <label className="flex items-center mb-2">
            <input
              ...
              className="mr-2"
            />
            <span className="text-sm font-medium text-gray-700">Enable Live Session</span>
          </label>
          {isLive && (
            <input
              type="datetime-local"
